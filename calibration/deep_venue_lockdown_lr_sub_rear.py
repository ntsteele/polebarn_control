...
Lockdown capture of 4 passes with XR18:
...
• Records from iRig (auto-detect by name, or override with env).
• NO EQ/delay changes; only LR/Bus5/Bus6 mute toggles.
• Quieter sweep level + more averaging for clean SNR at ~85–90 dB SPL.
"""
...
# helper (USB 17/18 playback + async)
...
# Sweep + level (quieter)
SWEEP_SEC    = 8.0            # keep long for LF timing; can reduce to 6.0 if needed
PREROLL_SEC  = 0.10
POSTROLL_SEC = 0.20
START_GAIN   = 0.015          # ↓ from 0.030
MAX_GAIN     = 0.040          # ↓ from 0.065 (hard ceiling)
MIN_PEAK     = 0.03           # accept slightly lower peaks at quieter level
N_PASSES     = 4              # ↑ more averaging for SNR
...
FALLBACK_INDEX   = 1  # your iRig was index 1
...
def pick_record_device():
    ...
    if REC_DEVICE_INDEX is not None:
        if devs[REC_DEVICE_INDEX]["max_input_channels"] < 1:
            raise RuntimeError("Selected device has no input.")
        print(f"🎙️ Using input index {REC_DEVICE_INDEX}: {devs[REC_DEVICE_INDEX]['name']}")
        return REC_DEVICE_INDEX
    hint = (REC_DEVICE_HINT or "").lower()
    for i,d in enumerate(devs):
        if d["max_input_channels"]>0 and hint in d["name"].lower():
            print(f"🎙️ Using input hint '{REC_DEVICE_HINT}': index {i} – {d['name']}")
            return i
    if FALLBACK_INDEX is not None and devs[FALLBACK_INDEX]["max_input_channels"]>0:
        print(f"🎙️ Using fallback index {FALLBACK_INDEX}: {devs[FALLBACK_INDEX]['name']}")
        return FALLBACK_INDEX
    for i,d in enumerate(devs):
        if d["max_input_channels"]>0:
            print(f"🎙️ Using first input device index {i}: {d['name']}")
            return i
...
def capture_avg(label, lbuf, rbuf, dev_index):
    gain = START_GAIN; takes=[]; peaks=[]
    while len(takes) < N_PASSES:
        seg, pk = record_one(lbuf*gain, rbuf*gain, dev_index)
        if pk < MIN_PEAK and gain*1.35 <= MAX_GAIN:
            gain *= 1.35
            print(f"⚠️  {label}: peak {pk:.3f} too low → gain→{gain:.3f} retrying")
            time.sleep(0.2); continue
        takes.append(seg); peaks.append(pk); time.sleep(0.2)
    return np.mean(np.stack(takes), axis=0), peaks
...
def main():
    ...
    avg, peaks = capture_avg("MainL", sweep, zeros, dev_index)
    np.save(folder/"MainL_response.npy", avg)
    results["MainL"] = {"avg_peak": float(np.mean(peaks)), "peaks":[float(p) for p in peaks]}
    print("✓ MainL")

    # 2) MAIN R
    avg, peaks = capture_avg("MainR", zeros, sweep, dev_index)
    np.save(folder/"MainR_response.npy", avg)
    results["MainR"] = {"avg_peak": float(np.mean(peaks)), "peaks":[float(p) for p in peaks]}
    print("✓ MainR")
    ...
    bus_on(BUS_SUB, True); rtn_to_bus(BUS_SUB, True)
    bus_on(BUS_REAR, False); rtn_to_bus(BUS_REAR, False)
    avg, peaks = capture_avg("Sub", sweep, sweep, dev_index)
    np.save(folder/"Sub_response.npy", avg)
    results["Sub"] = {"avg_peak": float(np.mean(peaks)), "peaks":[float(p) for p in peaks]}
    print("✓ Sub")
    ...
    bus_on(BUS_SUB, False); rtn_to_bus(BUS_SUB, False)
    bus_on(BUS_REAR, True);  rtn_to_bus(BUS_REAR, True)
    avg, peaks = capture_avg("Rear", sweep, sweep, dev_index)
    np.save(folder/"Rear_response.npy", avg)
    results["Rear"] = {"avg_peak": float(np.mean(peaks)), "peaks":[float(p) for p in peaks]}
    print("✓ Rear")
    ...
    print("   (No EQ/delay changed; only LR/Bus5/Bus6 mutes toggled.)\n")
